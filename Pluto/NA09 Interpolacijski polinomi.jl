...
using PlutoUI, Polynomials, Plots, LinearAlgebra, Random

# ╔═╡ d77dbe9f-e99a-4001-acbd-245fe48d64a9
...
begin
	...
	# iz paketa SpecialMatrices.jl
	struct Vandermonde{T} <: AbstractMatrix{T}
		c :: AbstractVector{T}
	end
	
	Base.getindex(V::Vandermonde, i::Int, j::Int) = V.c[i]^(j-1)
	isassigned(V::Vandermonde, i::Int, j::Int) = isassigned(V.c, i)
	
	Base.size(V::Vandermonde, r::Int) = (r==1 || r==2) ? length(V.c) :
	    throw(ArgumentError("Invalid dimension $r"))
	Base.size(V::Vandermonde) = length(V.c), length(V.c)
...
begin
	...
	xₜ=range(x₀,stop=xₙ,length=101)
	pS=p.(xₜ)
	plot(xₜ,pS,label="Polinom")
	scatter!(x,y,label="Točke")
end
...
Stoga se koriste i druge metode za računanje i izvrednjavanje interpolacijskih polinoma.
...
# ╔═╡ 0699b891-b23d-4479-8f32-c1d75779962d
# Bolje je formulirati koristeći skalarni produkt
L(t)=y⋅[prod(t .-x[[1:j-1;j+1:end]])/prod(x[j].-x[[1:j-1;j+1:end]]) 
        for j=1:n]

# ╔═╡ ef4cdc9e-7e9a-4b1a-b25c-c8e4266f8d6a
begin
	pL=Array{Float64}(undef,length(xₜ))
	for i=1:length(xₜ)
	    pL[i]=L(xₜ[i])
	end
end
...
begin
	pN=Array{Float64}(undef,length(xₜ))
	for i=1:length(xₜ)
	    pN[i]=evalnewton(c,x,xₜ[i])
	end
end

# ╔═╡ 2060e3ad-c95f-4cd6-99c6-673da7dd64ac
begin
	plot(xₜ,pN,label="Polinom")
	scatter!(x,y,label="Točke")
end
...
manifest_format = "2.0"
project_hash = "d9cb77293354c4c365b7ecc225bb5772d4283059"
